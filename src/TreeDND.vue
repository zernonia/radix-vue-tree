...
<template>
  <div class="flex justify-center w-full">
    <TreeRoot v-slot="{ flattenItems }"
      class="list-none select-none w-64 bg-white text-blackA11 rounded-md p-2 text-sm font-medium" :items="items"
      :get-key="(item) => item.title" multiple propagate-select>
      <TreeItem v-for="item in flattenItems" :key="item._id + item.index" :item="item" v-bind="item.bind"
